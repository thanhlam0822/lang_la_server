        ...
        try {
            ...
            switch (type) {
                ...
                case 5:
                    ...
                    break;
                case 6: // Quy đổi vàng sang bạc khóa
                    ...
                    if (matcher.matches()) {
                        try {
                            int number = Integer.parseInt(input);
                            if (number < 10) {
                                client.session.serivce.ShowMessGold("Số vàng tối thiểu là 10");
                            } else if (number > 100000) {
                                client.session.serivce.ShowMessGold("Số vàng tối đa là 100.000");
                            } else if (client.mChar.infoChar.vang < number) {
                                client.session.serivce.ShowMessGold("Không đủ vàng");
                            } else {
                                // Tính bạc khóa theo tỉ lệ bậc thang
                                long bacKhoa;
                                if (number >= 1000) {
                                    bacKhoa = (long) number * 50000;    // 1 vàng = 50.000
                                } else if (number >= 100) {
                                    bacKhoa = (long) number * 40000;    // 1 vàng = 40.000
                                } else {
                                    bacKhoa = (long) number * 30000;    // 1 vàng = 30.000
                                }

                                // Cộng bạc khóa trước — thành công mới trừ vàng
                                if (client.mChar.addBacKhoa(bacKhoa, true, true, "Đổi từ ngân khổ quan")) {
                                    client.mChar.mineVang(number, true, true, "Đổi từ ngân khổ quan");
                                    client.session.serivce.ShowMessGold("Đổi thành công "
                                            + Utlis.numberFormat(bacKhoa) + " bạc khóa");
                                } else {
                                    client.session.serivce.ShowMessGold("Có lỗi xảy ra, vui lòng thử lại");
                                }
                            }
                        } catch (NumberFormatException e) {
                            client.session.serivce.ShowMessGold("Số vàng không hợp lệ");
                        }
                    } else {
                        client.session.serivce.ShowMessGold("Số vàng không hợp lệ");
                    }
